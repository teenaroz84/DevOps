      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2, alignItems: 'stretch', '& > *': { minWidth: 0 } }}>
        <Paper elevation={0} sx={{ p: 2, minWidth: 0, overflow: 'hidden', border: '1px solid #e8ecf1', borderTop: `3px solid ${TRUIST.sky}`, borderRadius: 2, height: rowPanelHeight, display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ mt: 1, width: '100%', minWidth: 0, minHeight: 0, flex: 1, overflow: 'hidden', pb: 0.5 }}>
            <DataTable columns={warehouseEffCols} rows={data.warehouseCostEfficiency} maxHeight={136} tableMinWidth={1100} autoFitColumns={false} compact />
          </Box>
            ...
            {analyticsLoading && data.topSlowQueries.length === 0 ? (
              ...
              <>
                <Box sx={{ flex: 1, minHeight: 0, overflow: 'hidden', mb: 1 }}>
                  <DataTable columns={queryCols} rows={data.topSlowQueries} maxHeight={236} tableMinWidth={1200} autoFitColumns={false} compact />
                </Box>
